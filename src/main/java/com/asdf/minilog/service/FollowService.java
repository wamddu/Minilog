...
public class FollowService {
    ...
    public FollowResponseDto follow(Long followerId, Long followeeId) {
        ...
        User follower = userRepository.findById(followerId)
                .orElseThrow(() -> new UserNotFoundException(String.format("팔로어 아이디(%d)를 가진 사용자를 찾을 수 없습니다.", followerId)));

        User followee = userRepository.findById(followerId)
                .orElseThrow(() -> new UserNotFoundException(String.format("팔로잉 아이디(%d)를 가진 사용자를 찾을 수 없습니다.", followeeId)));
    ...
    public void unfollow(Long followerId, Long followeeId) {
        Optional<Follow> follow = Optional.ofNullable( followRepository.findByFollowerIdAndFolloweeId(followerId, followeeId)
                .orElseThrow(() -> new UserNotFoundException(String.format("팔로어(%d)와 팔로잉(%d)을 연결하는 Follow를 찾을 수 없습니다.", followerId, followeeId))));
